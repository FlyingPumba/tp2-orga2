...
ASM_hsl2:
	...
	.ciclo:
		...
		call rgbTOhsl ; xmm0 = |l|s|h|a|

		; hago la suma de floats
		addps xmm0, xmm15 ; xmm0 = |l+LL|s+SS|h+HH|a| (float)
		movdqu xmm5, xmm0 ; xmm5 = |l+LL|s+SS|h+HH|a| (copia parcial)
...
rgbTOhsl:
    ...
    punpcklbw xmm0, xmm14 ; xmm0 = | ceros | R | G | B | A | (cada canal en int 16b)
    punpcklwd xmm0, xmm14 ; xmm0 = | R | G | B | A | (cada canal en int 32b)
    ...
    movdqu xmm0, xmm14 ; xmm0 = | L | S | H | A |
	;*****
	pop rbp
	ret


	; void hslTOrgb(float *dst)
	hslTOrgb:
		push rbp
		mov rbp, rsp
		; **********
	    ...
	    ; cargo el pixel
	    movdqu xmm0, xmm5 ; xmm0 = | L | S | H | A | (floats SP)
		...
		pxor xmm14, xmm14
		packusdw xmm0, xmm14 ; xmm0 = | ceros | R | G | B | A | (int 16b)
		packuswb xmm0, xmm14 ; xmm0 = | ceros | R | G | B | A | (int 8b)

		; escribo el resultado
		movd [rdi], xmm0 ; [rsi] = | R | G | B | A |
		;*****
		pop rbp
		ret
