...
ASM_hsl1:
	...
	.ciclo:
		...
		; [rsp] = |l|s|h|a|
		movss xmm0, DWORD [rsp] ; xmm0 = |basura|basura|basura|l|
		movss xmm1, DWORD [rsp+4] ; xmm1 = |basura|basura|basura|s|
		movss xmm2, DWORD [rsp+8] ; xmm2 = |basura|basura|basura|h|

		; consigo los parametros de entrada
		; [rsp+16] = |LL|SS|HH|0.0|
		movss xmm3, DWORD [rsp+16] ; xmm3 = |basura|basura|basura|LL|
		movss xmm4, DWORD [rsp+20] ; xmm4 = |basura|basura|basura|SS|
		movss xmm5, DWORD [rsp+24] ; xmm5 = |basura|basura|basura|HH|
		...
		cmp eax, FALSE
		jnz .check_min
		subss xmm2, xmm6 ; xmm2 = |basura|basura|basura|h+HH-360.0|
		jmp .fin_ciclo
		...
		cmp eax, FALSE
		jnz .fin_ciclo
		addss xmm2, xmm6 ; xmm2 = |basura|basura|basura|h+HH+360.0|

		.fin_ciclo:

		movss DWORD [rsp], xmm0 ; [rsp] = |l_final|s|h|a|
		movss DWORD [rsp+4], xmm1 ; [rsp] = |l_final|s_final|h|a|
		movss DWORD [rsp+8], xmm2 ; [rsp] = |l_final|s_final|h_final|a|
